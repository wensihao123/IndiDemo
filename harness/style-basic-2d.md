...
>
> **本项目落地(test-2,已从 templates/ 提升填实)**:
> - 画风 = **平滑手绘风(非像素)** → 默认纹理过滤 **Linear**;导入按 §4.2"平滑风"预设。
> - 逻辑分辨率不是单一值:悬浮窗**宽随显示器变**,主游戏区固定 **800×250 居中**,
>   两侧占位背景平铺兜底。故 **Stretch mode = `disabled`(1:1 像素)**,不走 §4.4 默认的
>   `canvas_items` 缩放(见 01 PLAN.md D7)。本文凡 `<逻辑分辨率>` 处按"主区 800×250"理解。
> updated: 2026-06-17
>
> **版本**:机制基于 Godot **4.x**;本项目用 **4.6.x**。`TileMapLayer`、`Parallax2D`
> 为 4.3+ 引入。
>
...
- **本项目 = 平滑风 → `default_texture_filter = Linear`(已定死)。**
...
### 4.4 逻辑分辨率与拉伸
- **本项目特例:无单一逻辑分辨率。** 悬浮窗宽随显示器变,高恒 250,主区固定 800×250 居中。
  - `viewport_width/height` 给个种子值(如 1280×250),运行时按工作区宽覆盖窗口大小。
  - **Stretch `mode = disabled`(1:1 像素,靠锚点摆放),不用 `canvas_items` 整体缩放**——
    否则会把 800 主区也随屏宽缩放(违反"主区固定")。见 01 PLAN.md D7。
- 一般项目按下面默认走;本项目按上面特例:
