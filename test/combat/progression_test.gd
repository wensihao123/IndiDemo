extends GdUnitTestSuite
## PLAN step 4 验证:场景游标 0→1→2→Boss、Boss 击杀永久解锁 + 不再回到该 Boss(PLAN D4)。

func _enemy(hp: float) -> EnemyDef:
	var e := EnemyDef.new()
	e.max_hp = hp
	e.attack = 0.0          # 敌人不还手 → 战士不死,推进可控
	e.drop_chance = 0.0     # 不掺掉落随机
	return e

func _scene(hp: float, kills: int) -> SceneConfig:
	var s := SceneConfig.new()
	s.enemy = _enemy(hp)
	s.kill_count = kills
	return s

func _stage(p_name: String, kills: Array, boss_hp: float) -> StageConfig:
	var st := StageConfig.new()
	st.stage_name = p_name
	var arr: Array[SceneConfig] = []
	for k in kills:
		arr.append(_scene(1.0, k))
	st.scenes = arr
	st.boss = _enemy(boss_hp)
	return st

func _director() -> CombatDirector:
	var d: CombatDirector = auto_free(CombatDirector.new())
	# 强战士:1 击必杀 1 血怪,推进完全确定。
	d.party = [PartyMember.new("战士", 1000.0, 100.0), null, null, null]
	return d

func test_cursor_advances_through_scenes_to_boss() -> void:
	var d := _director()
	var stages: Array[StageConfig] = [_stage("第一关", [1, 1, 1], 500.0)]
	d.begin_run(stages)
	assert_int(d.cur_scene).is_equal(0)
	d.tick_combat()  # 杀场景0 → 场景1
	assert_int(d.cur_scene).is_equal(1)
	d.tick_combat()  # 杀场景1 → 场景2
	assert_int(d.cur_scene).is_equal(2)
	d.tick_combat()  # 杀场景2 → Boss
	assert_int(d.cur_scene).is_equal(CombatDirector.BOSS_SCENE)
	assert_int(d.cur_stage).is_equal(0)

func test_kill_count_gates_scene_advance() -> void:
	var d := _director()
	var stages: Array[StageConfig] = [_stage("第一关", [3, 1, 1], 500.0)]
	d.begin_run(stages)
	d.tick_combat()
	assert_int(d.cur_scene).is_equal(0)  # 仅 1/3 杀,未达标
	d.tick_combat()
	assert_int(d.cur_scene).is_equal(0)  # 2/3
	d.tick_combat()
	assert_int(d.cur_scene).is_equal(1)  # 3/3 达标 → 进场景1

func test_boss_kill_unlocks_next_stage_permanently() -> void:
	var d := _director()
	var stages: Array[StageConfig] = [
		_stage("第一关", [1], 1.0),   # 1 场景 + 1血 Boss
		_stage("第二关", [99], 500.0),
	]
	var cleared := [-1]
	d.boss_cleared.connect(func(s): cleared[0] = s)
	d.begin_run(stages)
	d.tick_combat()  # 杀场景0 → Boss
	assert_int(d.cur_scene).is_equal(CombatDirector.BOSS_SCENE)
	d.tick_combat()  # 杀 Boss → 解锁 + 进通关倒计时(不立刻推进,PLAN D6)
	assert_int(cleared[0]).is_equal(0)
	assert_int(d.max_unlocked_stage).is_equal(1)
	assert_int(d.mode).is_equal(CombatDirector.Mode.STAGE_CLEAR_COUNTDOWN)
	assert_int(d.cur_stage).is_equal(0)  # 倒计时内仍未推进
	d.process_countdown(d.stage_clear_countdown_sec + 0.1)  # 倒计时到点 → 自动推进
	assert_int(d.cur_stage).is_equal(1)
	assert_int(d.cur_scene).is_equal(0)
	assert_int(d.mode).is_equal(CombatDirector.Mode.PROGRESSING)

func test_never_refights_cleared_boss() -> void:
	var d := _director()
	var stages: Array[StageConfig] = [
		_stage("第一关", [1], 1.0),
		_stage("第二关", [99], 500.0),
	]
	d.begin_run(stages)
	d.tick_combat()  # 杀场景0 → Boss
	d.tick_combat()  # 杀 Boss → 倒计时
	d.process_countdown(d.stage_clear_countdown_sec + 0.1)  # 自动推进到第二关
	for i in 20:
		d.tick_combat()
	# 进了第二关后就停在第二关场景0(99 血打不完),绝不回到第一关 Boss。
	assert_int(d.cur_stage).is_greater_equal(1)

func test_party_heals_full_after_clearing_a_scene() -> void:
	# 战士 hp100/atk100;场景0 怪 hp150/atk10 → tick1 怪 150→50 还手(战士 100→90),
	# tick2 怪 50→0 死、清场进场景1 → 全队回满(过场景回血)。
	var d: CombatDirector = auto_free(CombatDirector.new())
	d.party = [PartyMember.new("战士", 100.0, 100.0), null, null, null]
	var st := StageConfig.new()
	st.stage_name = "回血关"
	var hurter := EnemyDef.new()
	hurter.max_hp = 150.0
	hurter.attack = 10.0
	hurter.drop_chance = 0.0
	var sc := SceneConfig.new()
	sc.enemy = hurter
	sc.kill_count = 1
	st.scenes = [sc, _scene(1.0, 1)] as Array[SceneConfig]
	st.boss = _enemy(1.0)
	d.begin_run([st] as Array[StageConfig])
	d.tick_combat()                                   # 怪 150→50,战士受 1 击
	assert_float(d.party[0].current_hp).is_equal(90.0)
	d.tick_combat()                                   # 怪 50→0 死,清场 → 回满
	assert_int(d.cur_scene).is_equal(1)
	assert_float(d.party[0].current_hp).is_equal(100.0)


func test_current_enemy_def_returns_boss_at_boss_scene() -> void:
	var d := _director()
	var boss_hp := 777.0
	var stages: Array[StageConfig] = [_stage("第一关", [1, 1, 1], boss_hp)]
	d.begin_run(stages, 0, CombatDirector.BOSS_SCENE)
	var def := d.current_enemy_def()
	assert_object(def).is_not_null()
	assert_float(def.max_hp).is_equal(boss_hp)
