...
func init_default_party() -> void:
	...
	party[0] = PartyMember.new(warrior_name, warrior_max_hp, warrior_attack)
...
## 推进一个战斗步:队伍齐攻 → 敌人若存活则反击最前成员 → 判定死亡 / 团灭。
func tick_combat() -> void:
	if not has_living_enemy() or not has_living_member():
		return
	# 队伍对当前敌人结算伤害(全体存活成员攻击之和;v1 = 1 战士)。
	_enemy_hp = maxf(0.0, _enemy_hp - _party_total_attack())
	if _enemy_hp <= 0.0:
		...
		if _progression_active:
			_advance_after_kill()
		return
	# 敌人反击最前存活成员(已被击败的敌人不反击)。
	var target := _front_living_member()
	if target != null:
		target.take_damage(_enemy_def.attack)
...
func _party_total_attack() -> float:
	var total := 0.0
	for m in party:
		if m != null and m.is_alive():
			total += m.attack
